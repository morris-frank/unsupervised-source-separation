...
from thesis.utils import range_product, remove_list_weight_norm
...
class Wavenet(nn.Module):
    def __init__(
        ...
        self.init_conv = nn.Conv1d(
            in_channels, residual_width, kernel_size, padding=(kernel_size - 1) // 2
        )
        ...
        for _, _ in range_product(self.n_blocks, self.n_layers):
            ...
            self.skip.append(wn(nn.Conv1d(residual_width, skip_width, 1, bias=False)))
            self.thru.append(
                wn(nn.Conv1d(residual_width, residual_width, 1, bias=False))
            )

            if self.conditional:
                self.gate_c.append(
                    wn(nn.Conv1d(c_channels, residual_width, 1, bias=False))
                )
                self.feat_c.append(
                    wn(nn.Conv1d(c_channels, residual_width, 1, bias=False))
                )
    ...
    def forward(
        self, x: torch.Tensor, conditional: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        ...
        for k in range(self.n_blocks * self.n_layers):
            dilated = dilate(h, new=self.dilations[k], old=self.dilations[k - 1])
