...
class WavenetVAE(nn.Module):
    def __init__(
        ...
        self.decoder_params = dict(
            ...
            conditional_dims=[(self.latent_width, 1), self.latent_width],
        )
    ...
    def encode(self, x: torch.Tensor, offsets: torch.Tensor) -> torch.Tensor:
        if self.z_c is None:
            self.z_c = torch.zeros(
                (x.shape[0], self.latent_width), device=x.device, dtype=torch.float32
            )
    ...
    def decode(self, x: torch.Tensor, z: List[torch.Tensor]) -> torch.Tensor:
        x = shift1d(x, -1)
        # TODO change cat to stack
        logits = [dec(x, z) for dec in self.decoders]
        return torch.cat(logits, dim=1)

    def loss(self, β: float = 1.0, dβ: float = 1 / 3) -> Callable:
        def func(model, x, y, progress):
            mixes, offsets = x
            logits, x_q, x_q_log_prob = model(mixes, offsets)

            # First Cross-Entropy
            ce_x = multi_cross_entropy(logits, y, len(self.decoders), self.out_channels)
...
class ConditionalWavenetVQVAE(nn.Module):
    ...
    def infer(self, x: torch.Tensor, labels: torch.Tensor, destroy: float = 0):
        z = self.encode(x, labels)
        if destroy > 0:
